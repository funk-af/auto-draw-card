...
import appSpec from '../artifacts/main/Main.arc56.json'
...
/**
 * Check if this is an asset supported by partner
 */
...
const subscriber = new AlgorandSubscriber(
  {
    filters: [
      ...
      // Subscribe to every event in the Main contract
      {
        name: 'mainEvent',
        filter: {
          type: algosdk.TransactionType.appl,
          arc28Events: appSpec.events.map((e) => {
            return { groupName: 'main', eventName: e.name }
          }),
        },
      },
    ],
    arc28Events: [{ groupName: 'main', events: appSpec.events }],
...
function handleMainEvent(tx: SubscribedTransaction) {
...
subscriber.onPoll((poll) => {
  ...
  poll.subscribedTransactions.forEach((tx) => {
    ...
    if (tx.filtersMatched?.includes('mainEvent')) handleMainEvent(tx)
  })
